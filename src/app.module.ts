...
import { neo4jConfig as neo4jConnectionConfig } from './config/neo4j.config';
...
import { UserSustainability } from './sustainability/entities/user-sustainability.entity';
import { UserStreak } from './profile/entities/user-streak.entity';
import { UserAchievement } from './profile/entities/user-achievement.entity';
  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        ...
        entities: [UserProfile, Medication, MedicationLog, DailyHealthMetric, Appointment, SocialEvent, UserSustainability, UserStreak, UserAchievement],
        synchronize: configService.get('app.environment') === 'development', // Enable sync in dev for rapid iteration
        logging: configService.get('app.environment') === 'development',
        ssl: configService.get('app.environment') === 'production'
          ? { rejectUnauthorized: true } // Proper SSL validation in production
